...
function handleResult(resultData) {
	...
	console.log("Single-genre.js: handleResult: populating star info from resultData");

    // populate the star info h3
    // find the empty h3 body by id "single_movie_info"
    let singleMovieInfoElement = jQuery("#single_genre_info");
...
//        "<p>Director: " + resultData[0]["movie_director"] + "</p>");
    ...
    console.log("Single-genre.js: handleResult: populating movie table from resultData");

    // Populate the single movie table
    // Find the empty table body by id "single_movie_table_body"
    let singleGenreTableBodyElement = jQuery("#single_genre_table_body");
    ...
    for (let i = 0; i < resultData.length; i++) {
        ...
        singleGenreTableBodyElement.append(rowHTML);
    }
}
...
console.log(`Single-genre.js: sending genreid = ${genreId} to api/single-genre`);
...
jQuery.ajax({
    ...
    url: "api/single-genre?id=" + genreId, 
